...
- Todas las rutas internas son **relativas sin slash inicial**
  (`blog/index.html`, no `/blog/index.html`). Esto funciona igual de bien
  sirviendo el sitio bajo una subruta (`usuario.github.io/psicocrecer/`) que
  bajo un dominio propio en la raíz (`www.psicocrecer.cl/`) — es justo lo que
  permitió cambiar de uno a otro sin tocar ningún enlace interno.
- Desde `blog/*.html`, las rutas a `assets/` deben subir un nivel:
  `../assets/css/main.css`.

## Dominio propio (`www.psicocrecer.cl`)

- El archivo **`CNAME`** en la raíz del proyecto (una sola línea:
  `www.psicocrecer.cl`) es obligatorio para que GitHub Pages sirva el sitio
  en el dominio propio — sin él, GitHub muestra un 404 aunque el DNS esté
  bien configurado. Si algún día se cambia de dominio, hay que editar este
  archivo Y actualizar el campo "Custom domain" en Settings → Pages del repo.
- Con dominio propio, el sitio se sirve **desde la raíz**, no desde
  `/psicocrecer/` — por eso todas las URLs absolutas (canonical, `og:url`,
  JSON-LD, `sitemap.xml`, `robots.txt`) usan `https://www.psicocrecer.cl/`
  seguido directamente del nombre de archivo.
- La configuración DNS (registro CNAME de `www` apuntando a
  `usuario.github.io`, o los registros A del dominio raíz) se hace en el
  proveedor de dominio (fuera de este repositorio) — no hay ningún archivo
  aquí que la controle.
...
- `og:url` / `canonical` / JSON-LD en las 13 páginas y en `sitemap.xml`/`robots.txt`
  ya usan el dominio real `https://www.psicocrecer.cl/` (sin subruta —
  ver sección "Dominio propio" más abajo sobre por qué no lleva `/psicocrecer/`).
